function make_constraints_matrix(obj, MLD_matrices, pos_vehs)
    ...
    if ~isfield(pos_vehs, 'north')
        pos_vehs_all = [pos_vehs.south; pos_vehs.east; pos_vehs.west];
    elseif ~isfield(pos_vehs, 'south')
        pos_vehs_all = [pos_vehs.north; pos_vehs.east; pos_vehs.west];
    elseif ~isfield(pos_vehs, 'east')
        pos_vehs_all = [pos_vehs.north; pos_vehs.south; pos_vehs.west];
    elseif ~isfield(pos_vehs, 'west')
        pos_vehs_all = [pos_vehs.north; pos_vehs.south; pos_vehs.east];
    end
    ...
    if ~isfield(pos_vehs, 'north')
        phase_groups(1) = {[1, 2, 3]};
        phase_groups(2) = {[1, 5, 6]};
        phase_groups(3) = {[3, 4, 5]};
        phase_groups(4) = {[1, 3, 5]};
    elseif ~isfield(pos_vehs, 'south')
        phase_groups(1) = {[1, 2, 5]};
        phase_groups(2) = {[1, 3, 4]};
        phase_groups(3) = {[3, 5, 6]};
        phase_groups(4) = {[1, 3, 5]};
    elseif ~ isfield(pos_vehs, 'east')
        phase_groups(1) = {[3, 5, 6]};
        phase_groups(2) = {[1, 2, 5]};
        phase_groups(3) = {[1, 3, 4]};
        phase_groups(4) = {[1, 3, 5]};
    elseif ~isfield(pos_vehs, 'west')
        phase_groups(1) = {[1, 5, 6]};
        phase_groups(2) = {[3, 4, 5]};
        phase_groups(3) = {[1, 2, 3]};
        phase_groups(4) = {[1, 3, 5]};
    end
    ...
    for phase_id = 1: obj.phase_num
        ...
        for step = 1:obj.N_p
            ...
            P_tmp(:, phase_group(1) + obj.v_num*(step-1)) = [-1; 0; 0; 1];
            P_tmp(:, phase_group(2) + obj.v_num*(step-1)) = [0; -1; 0; 1];
            P_tmp(:, phase_group(3) + obj.v_num*(step-1)) = [0; 0; -1; 1];
            P_tmp(:, obj.v_num*obj.N_p + phase_id + obj.phase_num*(step-1)) = [1; 1; 1; -1];
    ...
    for step = 1:obj.N_p
        ...
        P_tmp(1, 1 + obj.v_num*(step-1): obj.signal_num +obj.v_num*(step-1)) = [1, 1, 1, 1, 1, 1];
        obj.MILP_matrices.P = [obj.MILP_matrices.P; P_tmp];
    ...
    for step = 1:(obj.N_p-1)
        P_tmp = zeros(1, obj.variables_size);
        P_tmp(obj.v_num*obj.N_p + obj.phase_num*obj.N_p + (obj.phase_num + 1)*(step -1) + 1 : obj.v_num*obj.N_p + obj.phase_num*obj.N_p+ (obj.phase_num + 1)*step) = [-1, -1, -1, -1, 2];
        obj.MILP_matrices.Peq = [obj.MILP_matrices.Peq; P_tmp];
